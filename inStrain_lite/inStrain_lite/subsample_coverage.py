...
def calculate_pi(counts, min_cov = 5, subsample_coverage = 0):
    '''
    ...
    if subsample_coverage == 0:
        ...
        if s >= min_cov:
            if s >= subsample_coverage:
                P2C = {'A':0, 'C':1, 'T':2, 'G':3}
                C2P = {0:'A', 1:'C', 2:'T', 3:'G'}

                counts_prob = counts / np.sum(counts)

                sampled = np.random.choice(['A', 'C', 'T', 'G'], subsample_coverage, p=counts_prob)
                counts_sampled = np.zeros(4)
                for sample in sampled:
                    counts_sampled[P2C[sample]] += 1
...
def calculate_diversity(s, min_cov = 5, subsample_coverage = 0 ):
    ...
    for scaf in s.counts_table:
        ...
        for counts in scaf[:1000]:
            ...
            pi_table['pi'].append(calculate_pi(counts, subsample_coverage = subsample_coverage))
...
def calculate_gene_pi(gene_index, pi_table, sample, min_cov = 5, min_gene_size = 120):
    '''
    ...
    # Read in clonalities
    print("Calculating gene clonalities for " + sample)
    # Create gene column
...
def main(args):
    ...
    sample = args.input.split("/")[-1].split(".")[0]
    # load inStrain object
    s = SNPprofile()
    print("loading " + args.input)
    s.load(args.input)
    print("calculating nucleotide diversity in " + args.input)
    pi_table = calculate_diversity(s, subsample_coverage = int(args.subsample_coverage))
    print("calculating genes")
    g = calculate_gene_pi(gene_index, pi_table, sample, min_cov = 5, min_gene_size = 100)
    g.to_csv(sample + '.' + str(args.subsample_coverage) + '.genes.tsv', index=False, sep='\t')
...
if __name__ == '__main__':
    ...
    parser.add_argument("-u", "--subsample_coverage", action="store", default=0, \
        help='Coverage to subsample down to')
